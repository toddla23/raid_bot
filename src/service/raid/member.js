...
const addMember = async (partyId ,name, user_id, role) => {
  try {
    const [result, field] = await connection.query("INSERT INTO member (`party_id`, `name`, `user_id`, `role`) VALUES (?)", [[partyId, name, user_id, role]]);
    return result;
  } catch (e) {
    console.log(e);
  }
...
const findUserIdByParty = async (partyId) => {
  try {
    const [partyResult, partyField] = await connection.query("SELECT p.id, c.name, p.party_name FROM contents c, party p WHERE c.id = p.contents_id AND p.id = ?", [partyId]);
    const [dealerMemberResult, dealerMemberField] = await connection.query("SELECT `user_id` FROM member WHERE party_id = ? and role = 0", [partyId]);
    const [supporterMemberResult, supporterMemberField] = await connection.query("SELECT `user_id` FROM member WHERE party_id = ? and role = 1", [partyId]);
    if(partyResult.length !== 1)
      return;
...
const deleteByPartyAndName = async (partyId, userId) => {
  try{
    const [result, field] = await connection.query("DELETE FROM member WHERE party_id = ? and user_id = ?", [partyId, userId]);
    console.log(result);
    return result;
  }catch (e) {
    console.log(e)
...
const memberService = {addMember, findByParty, deleteByPartyAndName, findUserIdByParty};
