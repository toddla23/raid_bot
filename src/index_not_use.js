...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.commandName === '파티생성') {
    const result = await partyService.addParty(interaction.options.get("보스").value, interaction.options.get("파티명").value, interaction.options.get("시작시간").value);
    if (!result) {
      interaction.reply("error!")
      return;
    }
    // console.log(result);
    const startTime = interaction.options.get("시작시간").value;
    ...
    interaction.reply(` "${interaction.options.get("파티명").value}"파티가 생성 되었습니다.`)
  }
